...
int OS_AddPeriodicEventThreads(void(*thread1)(void), uint32_t period1,
  ...
  //***YOU IMPLEMENT THIS FUNCTION*****
		...
		SetInitialStack(4); Stacks[4][STACKSIZE-2] = (int32_t)(thread1);
		tcbs[4].period = period1;
		SetInitialStack(5); Stacks[5][STACKSIZE-2] = (int32_t)(thread2);
		tcbs[5].period = period2;
...
int32_t mailBoxSemaphore;
void OS_MailBox_Init(void){
  // include data field and semaphore
  //***YOU IMPLEMENT THIS FUNCTION*****
	mailBoxData = 0;
	OS_InitSemaphore(&mailBoxSemaphore,0);
